...
export function FlowerServerLogs() {
  ...
  const fetchLogs = async () => {
    ...
    // Debug API call for log fetching
    debugApiCall("FLOWER_CLIENT_LOGS", { podName, action: "fetchLogs" })

    try {
      // This is a simulation - in a real app, you would fetch logs from your K8s API
      const simulatedLogs = [
        `[${new Date().toISOString()}] Starting Flower client...`,
        `[${new Date().toISOString()}] Connecting to Flower server...`,
        `[${new Date().toISOString()}] Loading local dataset...`,
        `[${new Date().toISOString()}] Client ${podName} is running`,
      ...
      // Debug API call for log error
      debugApiCall("FLOWER_CLIENT_LOGS", { podName, action: "fetchLogs" }, { error })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const startStreaming = () => {
    ...
    // Debug API call for streaming start
    debugApiCall("FLOWER_CLIENT_LOGS", { podName, action: "startStreaming" })
    ...
    const interval = setInterval(() => {
      const newLog = `[${new Date().toISOString()}] Training round completed. Accuracy: ${(Math.random() * 0.3 + 0.7).toFixed(4)}`
  ...
  const stopStreaming = () => {
    ...
    // Debug API call for streaming stop
    debugApiCall("FLOWER_CLIENT_LOGS", { podName, action: "stopStreaming" })
  }
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-gray-900 text-white border-t border-gray-700">
      ...
      <div className="flex items-center justify-between px-4 py-2 relative">
        <div className="flex items-center">
          <h3 className="font-medium text-white">Flower Client</h3>
        </div>
