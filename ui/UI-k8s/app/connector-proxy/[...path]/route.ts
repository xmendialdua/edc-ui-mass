...
export async function GET(request: NextRequest, { params }: { params: { path: string[] } }) {
  try {
    ...
    // Cambiar api-proxy por connector-proxy
    const pathSegments = url.pathname.split("/connector-proxy/")[1]?.split("/") || []
    const path = pathSegments.join("/")

    // Verificar si es una solicitud PULL directa
    if (path === "pull-data") {
      // Obtener el endpoint personalizado del header
      const customEndpoint = request.headers.get("X-Pull-Endpoint")
      console.log(`[PULL DEBUG] X-Pull-Endpoint: ${customEndpoint}`)

      if (!customEndpoint) {
        return new NextResponse(JSON.stringify({ error: "Missing X-Pull-Endpoint header for PULL request" }), {
          status: 400,
          headers: {
            "Content-Type": "application/json",
          },
      ...
      const authHeader = request.headers.get("authorization")
      console.log(
        `[PULL DEBUG] Authorization header: ${authHeader ? authHeader.substring(0, 20) + "..." : "undefined"}`,
      )

      // Obtener la API key
      const connectorAddr = request.headers.get("X-Connector-Address") || getProxyConnectorAddress()
      const apiKey = getApiKeyForConnector(connectorAddr)
      console.log(`[PULL DEBUG] Using X-API-KEY: ${apiKey}`)

      // Obtener la dirección del conector
      const connectorAddress = request.headers.get("X-Connector-Address") || getProxyConnectorAddress()
      console.log(`[PULL DEBUG] Using connector address: ${connectorAddress}`)
      ...
      console.log(`[PULL DEBUG] Making direct PULL request to endpoint: ${customEndpoint}`)
      console.log(`[PULL DEBUG] Full token being used: ${authHeader}`)

      // Eliminar el prefijo "Bearer " si existe
      let finalAuthHeader = authHeader
      if (authHeader.startsWith("Bearer ")) {
        finalAuthHeader = authHeader.substring(7) // Eliminar "Bearer " del token
        console.log(`[PULL DEBUG] Removed 'Bearer ' prefix from token: ${finalAuthHeader.substring(0, 20)}...`)
      } else {
        console.log(`[PULL DEBUG] Token does not have 'Bearer ' prefix, using as is`)
      }

      // Realizar la solicitud PULL al endpoint con el token de autorización
      const pullResponse = await fetch(customEndpoint, {
        method: "GET",
        headers: {
          Authorization: finalAuthHeader, // Usar el token sin el prefijo "Bearer "
          "X-API-KEY": apiKey,
        },
      })
    ...
    if (path.includes("transfers") && path.includes("pull")) {
      ...
      // Añadir código para extraer el endpoint personalizado del header
      const customEndpoint = request.headers.get("X-Pull-Endpoint")

      // Si hay un endpoint personalizado, usarlo en lugar de construir la URL basada en el Connector Address
      let pullUrl
      if (customEndpoint) {
        // Usar directamente el endpoint personalizado
        pullUrl = customEndpoint
        console.log(`Using custom endpoint for PULL: ${pullUrl}`)
      } else {
        // Construir la URL como se hacía anteriormente
        const API_BASE_URL = request.headers.get("X-Connector-Address") || connectorAddress
        pullUrl = API_BASE_URL.replace("/api/management", "/public")
        console.log(`Using default constructed endpoint for PULL: ${pullUrl}`)
      }

      // Luego usar esta URL para hacer la solicitud al proveedor
      //const PULL_DATA_URL = pullUrl; // No es necesario, ya que handlePullRequest no usa PULL_DATA_URL directamente
...
export async function POST(request: NextRequest, { params }: { params: { path: string[] } }) {
  try {
    ...
    // Definir una variable local path para el bloque catch
    const currentUrl = error.url || ""
    const currentPath = currentUrl.includes("/request") ? "/request" : ""
...
export async function handler(req: NextRequest, { params }: { params: { path: string[] } }) {
  ...
  if (path.includes("/transfers/") && path.includes("/pull")) {
    // Extraer el endpoint personalizado del header si existe
    const customEndpoint = req.headers.get("X-Pull-Endpoint")

    // Usar el endpoint personalizado si está disponible, de lo contrario construir uno predeterminado
    let pullEndpoint
    if (customEndpoint) {
      console.log(`Using custom endpoint for PULL: ${customEndpoint}`)
      pullEndpoint = customEndpoint
    } else {
      // Construir el endpoint predeterminado (reemplazando /api/management por /public)
      pullEndpoint = connectorAddress.replace("/api/management", "/public")
      console.log(`Using default constructed endpoint: ${pullEndpoint}`)
    }

    console.log(`Handling PULL request for transfer ${path} to: ${pullEndpoint}`)

    // Resto del código para manejar la solicitud PULL...
  }
}
