...
microk8s kubectl get namespace iflex-provider > /dev/null 2>&1

# Si el namespace no existe, se crea
if [ $? -ne 0 ]; then
    echo "El namespace 'iflex-provider' no existe. Creando..."
    microk8s kubectl create namespace iflex-provider
else
    echo "El namespace 'iflex-provider' ya existe."
fi

# Aplicar los archivos YAML
microk8s kubectl apply -f k8s/rbac.yaml -n iflex-provider
microk8s kubectl apply -f k8s/configmap.yaml -n iflex-provider
microk8s kubectl apply -f k8s/deployment.yaml -n iflex-provider
microk8s kubectl apply -f k8s/service.yaml -n iflex-provider
