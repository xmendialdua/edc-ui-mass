...
// Asegúrate de que el campo de texto para la política tenga la clase font-mono para mejor visualización del JSON
// y que la validación de JSON funcione correctamente
...
export function PolicyFormDialog({ onAddPolicy }: PolicyFormProps) {
  ...
  // En el método handleSubmit, asegúrate de validar el JSON antes de enviar
  ...
  // Añadir esta función después de la función insertPolicyExample
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && !e.shiftKey && name.trim() && !jsonError) {
      e.preventDefault()
  ...
  return (
    ...
    >
      ...
      <DialogContent className="border-lime-200 sm:max-w-[600px]">
        ...
        <form onSubmit={handleSubmit} onKeyDown={handleKeyDown} className="space-y-4 mt-2">
