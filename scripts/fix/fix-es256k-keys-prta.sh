...
# Script para generar claves ES256K (secp256k1) reales para PRTA EDC
# Basado en documentación 20260422-resolucion-problema-refresco-token-edr.md

set -e

NAMESPACE="umbrella"
VAULT_POD="prta-edc-vault-0"
VAULT_TOKEN="root"
CONNECTOR_NAME="PRTA"
...
openssl ecparam -name secp256k1 -genkey -noout \
  -out /tmp/prta-token-signer-private-ec.pem
openssl pkcs8 -topk8 -nocrypt \
  -in /tmp/prta-token-signer-private-ec.pem \
  -out /tmp/prta-token-signer-private.pem
rm -f /tmp/prta-token-signer-private-ec.pem

# 2. Generar clave pública correspondiente
echo "2️⃣ Generando clave pública correspondiente..."
openssl pkey -in /tmp/prta-token-signer-private.pem \
  -pubout -out /tmp/prta-token-signer-public.pem

# 3. Verificar que son claves secp256k1
echo "3️⃣ Verificando que son claves secp256k1..."
CURVE=$(openssl pkey -in /tmp/prta-token-signer-private.pem -text -noout 2>&1 | grep -i "secp256k1" || true)
...
head -3 /tmp/prta-token-signer-private.pem | sed 's/^/      /'
echo "      ..."
echo ""
echo "   Clave Pública (primeras 3 líneas):"
head -3 /tmp/prta-token-signer-public.pem | sed 's/^/      /'
echo "      ..."
echo ""
...
if [ "$EXISTING_PRIVATE" != "changeme" ]; then
  ...
  echo "   📦 Guardando backup en /tmp/prta-vault-backup.json"
  kubectl exec -n $NAMESPACE $VAULT_POD -- \
    vault kv get -format=json secret/tokenSignerPrivateKey > /tmp/prta-vault-backup-private.json
  kubectl exec -n $NAMESPACE $VAULT_POD -- \
    vault kv get -format=json secret/tokenSignerPublicKey > /tmp/prta-vault-backup-public.json
else
...
kubectl cp /tmp/prta-token-signer-private.pem \
  "$NAMESPACE/$VAULT_POD:/tmp/prta-token-signer-private.pem"
kubectl exec -n $NAMESPACE $VAULT_POD -- sh -lc \
  'vault kv put secret/tokenSignerPrivateKey content="$(cat /tmp/prta-token-signer-private.pem)"'

echo "   → Cargando tokenSignerPublicKey..."
kubectl cp /tmp/prta-token-signer-public.pem \
  "$NAMESPACE/$VAULT_POD:/tmp/prta-token-signer-public.pem"
kubectl exec -n $NAMESPACE $VAULT_POD -- sh -lc \
  'vault kv put secret/tokenSignerPublicKey content="$(cat /tmp/prta-token-signer-public.pem)"'
...
VAULT_PRIVATE_CONTENT=$(kubectl exec -n "$NAMESPACE" "$VAULT_POD" -- \
  vault kv get -format=json secret/tokenSignerPrivateKey | \
  jq -r '.data.data.content')
printf '%s' "$VAULT_PRIVATE_CONTENT" > /tmp/prta-vault-verify-private.pem
VAULT_KEY_CURVE=$(openssl pkey -text -noout -in /tmp/prta-vault-verify-private.pem 2>&1 | grep -i "secp256k1" || true)
if [ -z "$VAULT_KEY_CURVE" ]; then
  echo "   ❌ ERROR: La clave almacenada en Vault NO es de curva secp256k1"
  echo "   Diagnóstico openssl:"
  openssl ec -text -noout -in /tmp/prta-vault-verify-private.pem 2>&1 \
    | grep -iE 'ASN1|OID|NIST|prime|curve|Field' | sed 's/^/      /' || true
  rm -f /tmp/prta-vault-verify-private.pem
  exit 1
fi
...
kubectl rollout restart deployment -n $NAMESPACE prta-edc-controlplane

echo "   → Reiniciando Data Plane..."
kubectl rollout restart deployment -n $NAMESPACE prta-edc-dataplane

echo ""
echo "⏳ Esperando a que los pods se reinicien..."
kubectl rollout status deployment -n $NAMESPACE prta-edc-controlplane --timeout=300s
kubectl rollout status deployment -n $NAMESPACE prta-edc-dataplane --timeout=300s
...
echo "      kubectl logs -n umbrella deployment/prta-edc-dataplane --tail=100 | grep -i 'key\\|token\\|ES256'"
echo ""
echo "   2. Verificar que NO hay errores:"
echo "      kubectl logs -n umbrella deployment/prta-edc-dataplane --tail=200 | grep -i error"
echo ""
...
echo "   kubectl exec -n umbrella prta-edc-vault-0 -- \\"
echo "     vault kv put secret/tokenSignerPrivateKey @/tmp/prta-vault-backup-private.json"
echo ""

# Limpiar archivos temporales locales y dentro del pod
rm -f /tmp/prta-token-signer-private.pem /tmp/prta-token-signer-public.pem
kubectl exec -n $NAMESPACE $VAULT_POD -- rm -f \
  /tmp/prta-token-signer-private.pem /tmp/prta-token-signer-public.pem 2>/dev/null || true
