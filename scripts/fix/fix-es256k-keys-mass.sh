...
openssl ec -in /tmp/mass-token-signer-private.pem \
...
CURVE=$(openssl ec -in /tmp/mass-token-signer-private.pem -text -noout 2>&1 | grep -i "secp256k1" || true)
...
PRIVATE_KEY=$(cat /tmp/mass-token-signer-private.pem | sed ':a;N;$!ba;s/\n/\\n/g')
PUBLIC_KEY=$(cat /tmp/mass-token-signer-public.pem | sed ':a;N;$!ba;s/\n/\\n/g')
...
VAULT_PRIVATE_CONTENT=$(kubectl exec -n "$NAMESPACE" "$VAULT_POD" -- \
  vault kv get -format=json secret/tokenSignerPrivateKey | \
  jq -r '.data.data.content')
printf '%b\n' "$VAULT_PRIVATE_CONTENT" > /tmp/mass-vault-verify-private.pem
VAULT_KEY_CURVE=$(openssl ec -text -noout -in /tmp/mass-vault-verify-private.pem 2>&1 | grep -i "secp256k1" || true)
