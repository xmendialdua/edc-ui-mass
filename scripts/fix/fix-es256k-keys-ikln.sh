...
# Script para generar claves ES256K (secp256k1) reales para IKLN EDC
# Basado en documentación 20260422-resolucion-problema-refresco-token-edr.md

set -e

NAMESPACE="umbrella"
VAULT_POD="ikln-edc-vault-0"
VAULT_TOKEN="root"
CONNECTOR_NAME="IKLN"
...
head -3 /tmp/ikln-token-signer-private.pem | sed 's/^/      /'
echo "      ..."
echo ""
echo "   Clave Pública (primeras 3 líneas):"
head -3 /tmp/ikln-token-signer-public.pem | sed 's/^/      /'
echo "      ..."
echo ""
...
if [ "$EXISTING_PRIVATE" != "changeme" ]; then
  ...
  echo "   📦 Guardando backup en /tmp/ikln-vault-backup.json"
  kubectl exec -n $NAMESPACE $VAULT_POD -- \
    vault kv get -format=json secret/tokenSignerPrivateKey > /tmp/ikln-vault-backup-private.json
  kubectl exec -n $NAMESPACE $VAULT_POD -- \
    vault kv get -format=json secret/tokenSignerPublicKey > /tmp/ikln-vault-backup-public.json
else
...
if [ -z "$VAULT_KEY_CURVE" ]; then
  ...
  openssl ec -text -noout -in /tmp/ikln-vault-verify-private.pem 2>&1 \
    | grep -iE 'ASN1|OID|NIST|prime|curve|Field' | sed 's/^/      /' || true
  rm -f /tmp/ikln-vault-verify-private.pem
  exit 1
fi
...
kubectl rollout restart deployment -n $NAMESPACE ikln-edc-controlplane

echo "   → Reiniciando Data Plane..."
kubectl rollout restart deployment -n $NAMESPACE ikln-edc-dataplane

echo ""
echo "⏳ Esperando a que los pods se reinicien..."
kubectl rollout status deployment -n $NAMESPACE ikln-edc-controlplane --timeout=300s
kubectl rollout status deployment -n $NAMESPACE ikln-edc-dataplane --timeout=300s
...
echo "      kubectl logs -n umbrella deployment/ikln-edc-dataplane --tail=100 | grep -i 'key\\|token\\|ES256'"
echo ""
echo "   2. Verificar que NO hay errores:"
echo "      kubectl logs -n umbrella deployment/ikln-edc-dataplane --tail=200 | grep -i error"
echo ""
...
echo "   kubectl exec -n umbrella ikln-edc-vault-0 -- \\"
