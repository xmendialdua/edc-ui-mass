        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Check MASS connector status if we're on data-management.html
            if (window.location.pathname.includes('data-management.html')) {
                checkMassConnectorStatus();
            }
        });
  ...
  --output downloaded-file.csv`;
        ...
        // Check MASS connector status and update indicator
        async function checkMassConnectorStatus() {
            const indicator = document.getElementById('mass-status-indicator');
            const statusText = document.getElementById('mass-status-text');
            
            // If elements don't exist, we're not on data-management.html
            ...
            try {
                const result = await callAPI('/phase1/check-connectivity', 'POST');
                
                // The endpoint checks both connectors, we only care about MASS
                // Look for MASS in the logs
                let massOk = false;
                if (result.logs) {
                    for (const log of result.logs) {
                        if (log.includes('MASS Management API: OK')) {
                            massOk = true;
                            break;
                        }
                    }
                ...
                console.error('Error checking MASS connector status:', error);
                indicator.className = 'status-indicator disconnected';
                statusText.className = 'status-text disconnected';
